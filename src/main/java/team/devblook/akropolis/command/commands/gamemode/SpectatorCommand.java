...
public class SpectatorCommand extends InjectableCommand {
    ...
    public void onCommand(CommandSender sender, String label, String[] args) {
        if (args.length == 0) {
            ...
            player.sendMessage(TextUtil.replace(Message.GAMEMODE_CHANGE.toComponent(), "gamemode", TextUtil.parse("SPECTATOR")));
            ...
            if (sender.getName().equals(player.getName())) {
                player.sendMessage(TextUtil.replace(Message.GAMEMODE_CHANGE.toComponent(), "gamemode", Component.text("SPECTATOR")));
            } else {
                player.sendMessage(TextUtil.replace(Message.GAMEMODE_CHANGE.toComponent(), "gamemode", Component.text("SPECTATOR")));
