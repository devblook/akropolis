...
public class SurvivalCommand extends InjectableCommand {
    ...
    public void onCommand(CommandSender sender, String label, String[] args) {
        if (args.length == 0) {
            ...
            player.sendMessage(TextUtil.replace(Message.GAMEMODE_CHANGE.toComponent(), "gamemode", TextUtil.parse("SURVIVAL")));
            ...
            player.sendMessage(TextUtil.replace(Message.GAMEMODE_CHANGE.toComponent(), "gamemode", Component.text("SURVIVAL")));
            sender.sendMessage(TextUtil.replace(TextUtil.replace(Message.GAMEMODE_CHANGE_OTHER.toComponent(), "player", player.name()), "gamemode", Component.text("SURVIVAL")));
            player.setGameMode(GameMode.SURVIVAL);
        }
